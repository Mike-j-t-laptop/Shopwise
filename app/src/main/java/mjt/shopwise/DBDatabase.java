...
import mjt.shopwise.DBDependency;
import mjt.shopwise.DBColumn;
import mjt.shopwise.DBIndex;
import mjt.shopwise.DBTable;
import mjt.shopwise.DBTrigger;
...
public class DBDatabase {
     ...
     */
    private ArrayList<DBTrigger> database_triggers;
    private String problem_msg;
    private ArrayList<DBDependency> dependencies;
    ...
    public DBDatabase() {
        ...
        this.database_triggers = new ArrayList<>();
    ...
    public DBDatabase (String dbname,
                       ArrayList<DBTable> tables,
                       ArrayList<DBIndex> indicies,
                       ArrayList<DBTrigger> triggers
                       ){
        this.usable = false;
        this.database_name = dbname;
        this.database_tables = tables;
        this.database_indicies = indicies;
        this.database_triggers = triggers;
        this.problem_msg = "";
        this.checkDBDatabaseIsUsable("DBDatabase (dbname, table, indicies, triggers)");
        this.buildDependecies();
    }
    ...
    private void buildDependecies() {
        for (DBIndex dbi: database_indicies) {
            dependencies.add(
                    new DBDependency(
                            dbi.getOwningtable(),
                            dbi,
                            DBDependency.DEPENDENCY_TABLE_INDEX)
            );
        }
        for (DBTrigger dbtrg: database_triggers) {
            dependencies.add(
                    new DBDependency(
                            dbtrg.getTriggerOwningTableAsDBTable(),
                            dbtrg,
                            DBDependency.DEPENDENCY_TABLE_TRIGGER
                    )
            );
        }
    }
    ...
    public void logDependencies() {
        ...
        boolean valid = false;
        String dependency_type = "Invalid";
        String parent_name = "Unknown";
        String parent_type = "Unknown";
        String child_name = "Unknown";
        ...
        for (DBDependency d: dependencies
            ...
            switch (d.getDependency_type()){
                case DBDependency.DEPENDENCY_TABLE_INDEX:
                    dependency_type = "INDEX";
                    parent_name = getObjectName(d.getParent());
                    parent_type = DBTable.class.getSimpleName();
                    child_name = getObjectName(d.getChild());
                    child_type = DBIndex.class.getSimpleName();
                    break;
                case DBDependency.DEPENDENCY_TABLE_TRIGGER:
                    dependency_type = "TRIGGER";
                    parent_name = getObjectName(d.getParent());
                    parent_type = DBTable.class.getSimpleName();
                    child_name = getObjectName(d.getChild());
                    child_type = DBTrigger.class.getSimpleName();
                case DBDependency.DEPENDENCY_TABLE_REFERENCE:
                    dependency_type = "FK REFERENCE (unsupported as yet)";
                    break;
            }
            Log.d("DEPENDENCYINFO", "Type = " +
                    dependency_type +
                    " Parent Name(type) = " +
    ...
    private String getObjectName(Object o) {
        if (o instanceof DBTable) {
             return ((DBTable) o).getDBTableName();
        }
        if (o instanceof  DBIndex) {
            return ((DBIndex) o).getIndexName();
        }
        if (o instanceof DBTrigger) {
            return ((DBTrigger) o).getTriggerName();
        }
        return "Unknown";
    }
}
